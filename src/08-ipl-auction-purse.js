...
export function iplAuctionSummary(team, players) {
  ...
  // var role =["bat","bowl","ar","wk"];
  const totalSpent = players.reduce((acc,player)=>{
    return acc + player.price;},0);
  
  const costliestPlayer = players.reduce((max,player)=>{
    return player.price>max.price?player :max;}, players[0]);

  const cheapestPlayer = players.reduce((min,player)=>{
    return player.price<min.price?player :min;}, players[0]);
  ...
  return {
    teamName:team.name, totalSpent, remaining,
  playerCount,
  costliestPlayer,
  cheapestPlayer,
  averagePrice,
